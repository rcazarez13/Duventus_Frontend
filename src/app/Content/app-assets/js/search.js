...
$(function () {
   ...
   //-------------
    ...
    var $file_locale = $(".header-search-wrapper .header-search-input,.search-input-sm .search-box-sm").data("locale");
    var site_list;
    $.getJSON("/Content/app-assets/data/" + $file_locale +"." + $filename + ".json", function (data) {
        site_list = data;
    });
   ...
   $(".header-search-wrapper .header-search-input, .search-input-sm .search-box-sm").on("keyup", function (e) {
      ...
      if (e.keyCode !== 38 && e.keyCode !== 40 && e.keyCode !== 13) {
         ...
         if (value != "") {
            ...
            for (var i = 0; i < site_list.listItems.length; i++) {
                // Search list item start with entered letters and create list
                if (
                    (site_list.listItems[i].name.toLowerCase().indexOf(value) == 0 && a < 4) ||
                    (!(site_list.listItems[i].name.toLowerCase().indexOf(value) == 0) &&
                    site_list.listItems[i].name.toLowerCase().indexOf(value) > -1 &&
                    a < 4)
                ) {
                    if (a === 0) {
